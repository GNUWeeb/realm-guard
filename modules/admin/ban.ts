...
export const banCommand = {
  ...
  function: async (ctx: any) => {
    ...
    else if (!ctx.message.reply_to_message && ctx.message.text.split(" ")[1].startsWith("@")) {
      await ctx.reply("The API doesn't allow me to ban users by username, please reply to a message or type the user ID to ban a user!");
      console.log("[ERROR] No valid reply or user ID!");
      return;
    }
    ...
    // Ban user by reply
      ...
      console.log(`[BAN] ${ctx.message.reply_to_message.from?.first_name} (${ctx.message.reply_to_message.from?.id})`);
      await ctx.reply(`User ${ctx.message.reply_to_message.from?.first_name} (${ctx.message.reply_to_message.from?.id}) banned!`);
    }
...
export const unbanCommand = {
  ...
  function: async (ctx: any) => {
    ...
    // Check if user trying to unban by username since you simply can't
      ...
      await ctx.reply("The API doesn't allow me to unban users by username, please reply to a message or type the user ID to unban a user!");
      console.log("[ERROR] No valid reply or user ID!");
      return;
    } 
