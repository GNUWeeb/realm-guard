...
export const muteCommand: Command = {
  ...
  function: async (ctx) => {
    ...
    if (adminList.some((admin) => admin.user.id === ctx.message.reply_to_message?.from?.id)) {
      await ctx.reply("I'm sorry, but you can't mute an admin/owner of this group.");
    ...
    // Mute user by reply
      ...
      console.log(`[MUTE] ${ctx.message.reply_to_message.from?.first_name} (${ctx.message.reply_to_message.from?.id})`);
      await ctx.reply(`User ${ctx.message.reply_to_message.from?.first_name} (${ctx.message.reply_to_message.from?.id}) muted!`);
    }
...
export const unmuteCommand: Command = {
  ...
  function: async (ctx) => {
    ...
    if (adminList.some((admin) => admin.user.id === ctx.message.reply_to_message?.from?.id)) {
      await ctx.reply("I'm sorry, but you can't unmute an admin/owner of this group.");
      console.log("[ERROR] User is an admin!");
      return;
    }
    ...
    // Unmute user by reply
      ...
      console.log(`[UNMUTE] ${ctx.message.reply_to_message.from?.first_name} (${ctx.message.reply_to_message.from?.id})`);
      await ctx.reply(`User ${ctx.message.reply_to_message.from?.first_name} (${ctx.message.reply_to_message.from?.id}) unmuted!`);
    }
    ...
    // Unmute user by ID
      ...
      await ctx.reply(`User ${user.user.first_name} (${user.user.id}) unmuted!`);
    }
  }
}