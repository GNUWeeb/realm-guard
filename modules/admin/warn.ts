...
async function validate_request(ctx: any, bitmask: number)
{
        ...
        if ((bitmask & U_MUST_BE_IN_SUPERGROUP) && message?.chat?.type !== "supergroup") {
                await replyToMsgId(ctx, "This command can only be executed in a supergroup!", msg_id);
                return false;
        }

        if ((bitmask & U_MUST_REPLY) && !message?.reply_to_message) {
                await replyToMsgId(ctx, "You need to reply to a message to use this command.", msg_id);
                return false;
        }

        const admin_list = await ctx.getChatAdministrators();

        if ((bitmask & U_MUST_BE_AN_ADMIN) && !admin_list.some((admin: any) => admin.user.id === ctx.from?.id)) {
                await replyToMsgId(ctx, "You are not an admin!", msg_id);
                return false;
        }

        if ((bitmask & U_CANT_REPLY_TO_ADMIN) &&
             admin_list.some((admin: any) => admin.user.id === message.reply_to_message?.from?.id)) {
                await replyToMsgId(ctx, "You can't warn an admin/owner of this group.", msg_id);
                return false;
        }
...
{
        const mask = U_MUST_REPLY | U_MUST_BE_IN_SUPERGROUP | U_MUST_BE_AN_ADMIN
        ...
        if (!await validate_request(ctx, mask))
                return;
